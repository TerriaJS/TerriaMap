...
This is for the magda platform admin to manage terria aspects and all tenants, e.g., via URL https://admin-withterria.dev.magda.io/terriaAdmin.

### Tenant website management UI
This is for each tenant website's admin to configure and manage their own website. For example, The admin of website https://web1-withterria.dev.magda.io 
...
1. Check out the `withTerria` branch.
...
4. The gitlab job deploys magda with multi-tenant mode disabled by default. After the deployment, if you browse https://withterria.dev.magda.io, a magda 
   website will appear (without any datasets). We need to redeploy it in multi-tenant mode and speciify addtional arguments.
  ...
  - host: web2-withterria.dev.magda.io
    http:
      ...
      - backend:
        ...
        path: /
```
Also replace the `- withterria.dev.magda.io` in `hosts` of `tls` section with `- admin-withterria.dev.magda.io`. The final text looks like:
```
apiVersion: extensions/v1beta1
kind: Ingress
metadata:
...
### Magda admin tasks
#### Login as admin user
Before an admin UI and a login UI become available, we have to manually create user account then login onto magda. Magda uses third-party authentication service, such as `data.gov.au`, facebook and google. 
...
##### Login
Open a Chrome browser and navigate to https://admin-withterria.dev.magda.io/auth/login/ckan.
A very primative UI will appear. Fill in your data.gov.au username and password then submit. If successful, "You are logged in. View your profile." will be displayed. Click on `profile` to confirm that
`isAdmin` value is `true`. Otherwise you will not have write access to the resources.
...
#### Add terria aspects
Use Postman, post the following json data to https://admin-withterria.dev.magda.io/api/v0/registry/aspects.
...
#### Add tenants
Post the following json data to https://admin-withterria.dev.magda.io/api/v0/registry/tenants.
...
After adding the two tenants, we need to make a GET request to https://admin-withterria.dev.magda.io/refreshTenants to "activate" them. (This step will not be needed in the future.)
...
#### Login
Open a Chrome browser and navigate to https://web1-withterria.dev.magda.io/auth/login/ckan.
A very primative UI will appear. Fill in your data.gov.au username and password then submit. If successful, "You are logged in. View your profile." will be displayed. Click on `profile` to confirm that
`isAdmin` value is `true`. Otherwise you will not have write access to the resources.
