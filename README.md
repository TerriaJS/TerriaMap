  ...
  - host: web2-multi-tenant.dev.magda.io
    http:
      ...
      - backend:
        ...
        path: /
```
Also replace the "- multi-tenant.dev.magda.io" in "hosts" of "tls" section with "- admin-multi-tenant.dev.magda.io". Save the final text and close the editor.
```
apiVersion: extensions/v1beta1
kind: Ingress
metadata:
...
status:
  loadBalancer:
    ...
    - ip: 35.201.203.227
```
Now all the above three URLs will resolve to the gateway external IP address.

### Magda admin tasks
The magda admin needs to add some terria aspects and some tenants in the registry database.
Before an admin portal UI becomes available, we have to use other tools such as Postman to do the jobs. 
...
Tenant website admins are responsible to configure their own terria websites. Before the website admin portal UI
...
### Security
Recall that in Step 2 of [Deploy Magda to Google Cloud](#Deploy-Magda-to-Google-Cloud), we set registry-api: skipAuthorization to true in the file "magda/deploy/helm/preview.yml", which allows us to add aspects, tenants and tenant records without authentication. This is OK during the development stage. After adding those data, we should set its value to false then redeploy. Note that we need to set current context first.
```
...
```
Now all write accesses require authentication. For example, you will get message "The resource requires authentication, which was not supplied with the request" if you try to [add tenants](#Add-tenants).
