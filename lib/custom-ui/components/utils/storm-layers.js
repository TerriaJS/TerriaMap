  ...
  if (!layer) {
    ...
    getTrackData(year, stormnumber, advisory).then((track) => {
      if (track != null) {
        ...
        // copy info object from this item
        let info = item.info;
        //const info = item.getTrait(CommonStrata.user, "infoAsObject")
        //let info = structuredClone(item.info)
...
function createComposite(icon_url, new_id, terria, info) {
  ...
  //composite.setTrait(CommonStrata.user, "infoAsObject", { ProductType: 'hurr_composite' })
  // Create info section for the composite - this code doesn't seem to work
  // const info = createStratumInstance(InfoSectionTraits, [{
  //   name: "ProductType",
  //   content: "hurr_composite"
  // }]);

  // set info ProductType to "hurr_composite" so layer-card is created correctly
  // i know this is not working because info is a reference to the info object
  // in the original ietm layer, and it will change that one too.
  info["ProductType"] = "hurr_composite";
  composite.setTrait(CommonStrata.user, "info", info);
