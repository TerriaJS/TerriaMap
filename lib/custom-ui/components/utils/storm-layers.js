...
export function addStormLayers(viewState, item) {
  ...
  let new_id = item_id + "-hurr-composite";
  ...
  if (!layer) {
    ...
    getTrackData(year, stormnumber, advisory).then((track) => {
      if (track != null) {
        ...
        // now add points, track, and cone
        // geojson catalog items to the workbench
        // create a composite type catalog item to store
        // all of the hurricanes layers

        let composite = createComposite(icon_url, new_id, viewState.terria);
        ...
        LAYER_TYPES.forEach((layer_type) => {
          ...
          //     "stroke-opacity": 0.8,
          //   });
          //   geojson_item.setTrait(CommonStrata.user, "show", false)
          ...
          //geojson_item.setTrait(CommonStrata.user, "show", true)
...
function createComposite(icon_url, new_id, terria) {
