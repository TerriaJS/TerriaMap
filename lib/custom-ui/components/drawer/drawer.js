...
export const Drawer = ({ viewState }) => {
  ...
  const { activeLayer, layerSelection, layers, setLayers } = useLayers();
  ...
  return (
    ...
    >
      <Button
        ...
        variant="contained"
      >
        - {layerSelection.length} layers selected -
      </Button>

      <Stack direction={{ xs: "column", md: "row" }} className="content">
        <Box className="layers-list">
          <LayersList layers={layers} viewState={viewState} />
        </Box>
        <Box xs={12} md={9} className="layers-details">
          {activeLayer ? <ActiveLayerDetails /> : <GeneralDetails />}
        </Box>
      </Stack>
    </MuiDrawer>
  );
